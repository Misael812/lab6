...
void test_coop_busy_busy_and_sleep(){
    uint64_t workerOneTime, workerTwoTime, elapsedTime;
    supervisor_execution_analyzer((k_thread_entry_t)busy_yield, (k_thread_entry_t)busy_sleep, K_PRIO_COOP(4), K_PRIO_COOP(3),
                                    K_MSEC(10), K_MSEC(10), &workerOneTime, &workerTwoTime, &elapsedTime);

    TEST_ASSERT_UINT64_WITHIN(100000, 5000000, workerOneTime);
    TEST_ASSERT_UINT64_WITHIN(1000, 10000, workerTwoTime);
}

void test_preempt_busy_busy_and_sleep(){
    uint64_t workerOneTime, workerTwoTime, elapsedTime;
    supervisor_execution_analyzer((k_thread_entry_t)busy_yield, (k_thread_entry_t)busy_sleep, K_PRIO_PREEMPT(4), K_PRIO_PREEMPT(3),
                                    K_MSEC(10), K_MSEC(10), &workerOneTime, &workerTwoTime, &elapsedTime);
...
{
    UNITY_BEGIN();
    RUN_TEST(test_coop_busy_busy_and_sleep);
    RUN_TEST(test_preempt_busy_busy_and_sleep);
    return UNITY_END();
}